...
async function route(
  ...
  if (path === 'upscale') {
    ...
    const response = await fetch(ENDPOINTS.upscale, {
      ...upscaleRequest(token, input.toString('base64')),
      signal
    })
    if (!response.ok) {
      const detail = await response.text().catch(() => '')
    ...
    if (!entry) throw new Error('Upscale response did not contain an image')
    return { base64: await zip.file(entry)!.async('base64') }
  }
