...
function uniqueFolderName(existingNames: ReadonlySet<string>, base: string): string {
  if (!existingNames.has(normalizeFragmentPath(base))) return base
  for (let i = 2; ; i++) {
    const candidate = `${base} (${i})`
    if (!existingNames.has(normalizeFragmentPath(candidate))) return candidate
  }
}
...
export function planDefaultDanbooruSeed(
  ...
  const fragmentNames = new Set([...existingFragmentNames].map(normalizeFragmentPath))
  const missing = DEFAULT_DANBOORU_FRAGMENTS.filter(
    (fragment) => !fragmentNames.has(normalizeFragmentPath(fragment.name))
  )
  if (missing.length === 0) return null

  return {
    folderName: uniqueFolderName(
      new Set([...existingFolderNames].map(normalizeFragmentPath)),
